...
def SaveH5File_C(resize_size):
    ...
    data = h5py.File(path)
    with h5py.File('./data/MSCOCO_deep_doc2vec_data.h5py', 'r') as f:
        print(list(f.keys()))
    fi = np.concatenate([data['XAll'][()]], axis=0)
    fl = np.concatenate([data['LAll'][()]], axis=0)
    ft = np.concatenate([data['YAll'][()]], axis=0)
    imgs = list(fi[query_size: query_size + train_size])
    labs = list(fl[query_size: query_size + train_size])
    tags = list(ft[query_size: query_size + train_size])
    n = len(imgs)
...
def SaveH5File_N(resize_size):
    ...
    query_size = 2100
    root = './data/'
    fi = sio.loadmat(root + 'nus-wide-tc10-xall-vgg-clean.mat')['XAll']
    fl = sio.loadmat(root + 'nus-wide-tc10-lall-clean.mat')['LAll']
    ft = sio.loadmat(root + 'nus-wide-tc10-yall-clean.mat')['YAll']
    imgs = list(fi[query_size: query_size + train_size])
    labs = list(fl[query_size: query_size + train_size])
    tags = list(ft[query_size: query_size + train_size])
    n = len(imgs)
...
def SaveH5File_I(resize_size):
    ...
    fi, ft, fl = np.concatenate([valid_img, test_img]), np.concatenate([valid_txt, test_txt]), np.concatenate(
...
def SaveH5File_W(resize_size):
    root = './data/'
    file_path = os.path.join(root, 'WikiData.mat')
    data = sio.loadmat(file_path)
    valid_img = data['I_tr'].astype('float32')
    valid_txt = data['T_tr'].astype('float32')
    valid_labels = data['L_tr']
    test_img = data['I_te'].astype('float32')
    test_txt = data['T_te'].astype('float32')
    test_labels = data['L_te']
    fi, ft, fl = np.concatenate([valid_img, test_img]), np.concatenate([valid_txt, test_txt]), np.concatenate(
        [valid_labels, test_labels])
    query_size = 693
    train_size = 2173
...
def get_data(config):
    ...
    for data_type in ["train", "test", "database"]:
        dsets[data_type] = DataList(config["dataset"], data_type,
                                    transforms.ToTensor(), config["noise_type"], config["noise_rate"],
                                    config["random_state"])
...
def calc_map_k(rB, qB, retrieval_label, query_label, k=None):
    ...
    qB, rB, query_label, retrieval_label = [i.cuda().to(dtype=torch.float64) for i in
...
def plot_gmm(gmm, X, clean_index, noisy_index, save_path='', plot_pdf=True):
    ...
    # Plot data histogram
    ax.hist(X[clean_index], bins=100, density=True, histtype='stepfilled', color='green', alpha=0.4,
            label='Clean Pairs')
